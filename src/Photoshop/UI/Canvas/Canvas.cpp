...
void Canvas::OnToolChangedEvent(const ToolChangedEvent &event) {
    Tool& active_tool = palette_.GetActiveTool();
    active_tool.OnConfirm(Vector2(0, 0), *this);
}
...
void Canvas::OnKeyEvent(const KeyEvent &event) {
    ...
    if (event.Keys[KeyCode::Escape].Pressed) {
        active_tool.OnCancel(Vector2(0, 0), *this);
    }
}
...
void Canvas::DrawPen(const MouseEvent &event) {
    auto old_mouse_pos = MapFromGlobal(event.MouseOldPosition);
    auto mouse_pos = MapFromGlobal(event.MousePosition);

    sf::Color pen_color(sf::Color::Green);

    sf::Vertex line[] {
        sf::Vertex(old_mouse_pos),
        sf::Vertex(mouse_pos)
    };
    // vertexs_.push_back(sf::Vertex(texture_.mapPixelToCoords(old_mouse_pos)));
    // vertexs_.push_back(sf::Vertex(texture_.mapPixelToCoords(mouse_pos)));
}

void Canvas::ResetActiveFigure() {
    active_figure_->figure->SetDedicated(false);
    active_figure_ = nullptr;
}

Figure *Canvas::GetActiveFigure() {
    if (!active_figure_) return nullptr;
    return active_figure_->figure.get();
}

void Canvas::DrawFigure(Figure *figure) {
    Node* node = new Node(figure, nullptr, figures_tail_);
    if (figures_head_ == nullptr) {
        figures_head_ = figures_tail_ = node;
    } else {
        figures_tail_->next = node;
        figures_tail_ = node;
    }

    active_figure_ = node;
    active_figure_->figure->SetDedicated(true);
}

void Canvas::DeleteActiveFigure() {
    if (!active_figure_) return;

    if (figures_head_ == figures_tail_) {
        figures_head_ = figures_tail_ = nullptr;
        delete active_figure_;
        active_figure_ = nullptr;
    } else {
        figures_tail_ = figures_tail_->prev;
        figures_tail_->next = nullptr;
        delete active_figure_;
        active_figure_ = nullptr;
    }
}
